...
class Enemy(Sprite):
    def __init__(self, game):
        ...
        self.game=game
        num_start_pos = randint(1, 4)
        if num_start_pos == 1:
            self.rect.centerx = randint(0, self.setting.screen_width)
            ...
            self.rect.centery = randint(0, self.setting.screen_height)
        else:
            self.rect.centerx = self.setting.screen_width + 40
            self.rect.centery = randint(0, self.setting.screen_height)
        self.pos = pg.Vector2(float(self.rect.centerx), float(self.rect.centery))
        self.move_to_pos = pg.Vector2(randint(0, self.setting.screen_width),
                                      randint(0, self.setting.screen_width))
